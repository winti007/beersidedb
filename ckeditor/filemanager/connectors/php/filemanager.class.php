...
class Filemanager {
  ...
  function tolthet($mit)
  {
        $mit=strtolower($mit);
        $Kit=substr($mit,strrpos($mit,"/")+1,strlen($mit));
        $Vanphp=strpos($mit,"php");
        if ($Vanphp===false)return true;
        return false;
  ...
  public function add() {
    ...
    if (!($this->tolthet($_FILES['newfile']['name'])))
    {
      $this->error(sprintf($this->lang('INVALID_FILE_UPLOAD')),true);
    }
  ...
  private function cleanString($string, $allowed = array()) {
    ...
    $mapping = array(
        ...
        'O'=>'O', 'Ö'=>'O', 'Ő'=>'O', 'O'=>'O', 'U'=>'U', 'Ú'=>'U', 'U'=>'U', 'Ü'=>'U', 'Ű'=>'U', 'Ý'=>'Y',
        '?'=>'B', 'ß'=>'Ss','a'=>'a', 'á'=>'a', 'â'=>'a', 'a'=>'a', 'ä'=>'a', 'a'=>'a', 'a'=>'a', 'ç'=>'c',
        'e'=>'e', 'é'=>'e', 'e'=>'e', 'ë'=>'e', 'i'=>'i', 'í'=>'i', 'î'=>'i', 'i'=>'i', '?'=>'o', 'n'=>'n',
        'o'=>'o', 'ó'=>'o', 'ô'=>'o', 'o'=>'o', 'ö'=>'o', 'ő'=>'o', 'o'=>'o', 'u'=>'u', 'ú'=>'u', 'ű'=>'u',
        'u'=>'u', 'ý'=>'y', 'ý'=>'y', '?'=>'b', 'y'=>'y', 'Ŕ'=>'R', 'ŕ'=>'r', ' '=>'_', "'"=>'_', '/'=>''
        );
  ...
  private function availableLanguages() {
    ...
    if ($handle = opendir($this->root.'/scripts/languages/')) {
      while (false !== ($file = readdir($handle))) {
        if ($file != "." && $file != "..") {
          array_push($this->languages, pathinfo($file ));
        }
      }
      closedir($handle);
    }
